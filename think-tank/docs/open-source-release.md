...
当前默认公开发行档是 `skill_core_only_bundle`。In other words, the current default public release is `skill_core_only_bundle`。`leader-runtime` 应作为独立 sibling 项目存在，而不是继续留在当前 Skill 仓库的默认公开包里。
