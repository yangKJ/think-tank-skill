...
- `../self-tests/`：仓库级公开 self-test fixtures，用来检查 trigger、anti-trigger、provider boundary、composition 和 memory write 边界。
...
领域包层已迁至 `.think-tank/domain-packs/`（本地配置，不随公开 Skill 发布）。当前主仓不内置任何私有领域包；具体项目可以在自己的 `.think-tank/domain-packs/` 中添加 domain pack 或本地资料。
...
- `../starter-kits/research-workspace/`：Research OS starter kit。
...
4. `examples/usage/research-request.md`
5. `examples/usage/council-decision.md`
6. `examples/usage/review-acceptance.md`
