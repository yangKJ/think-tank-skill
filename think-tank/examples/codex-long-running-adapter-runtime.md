...
dispatch_decision:
  ...
  provider_selected: research-to-video-production
...
1. `research-to-video-production` 在当前机器上完成了真实 provider invocation，而不是只有 policy selection。
...
该样例可以把 `research-to-video-production` 提升为公开 `verified_partial` provider 证据，并证明：
