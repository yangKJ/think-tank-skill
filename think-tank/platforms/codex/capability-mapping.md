...
旧 research agent 的工具型 skills 可以按同级关系安装到 Codex skills 目录。Codex adapter 通过 `provider-registry.md` 和 `runtime/provider_registry.py` 把当前项目实际存在的 `.codex/skills/` 转换为 `available_providers[]`。think-tank 可以把这些 provider 视为 optional 候选实现，但必须通过 `routing/skill-router.md` 和 `routing/dispatch-policy.md` 逐次选择和验证执行能力。
...
4. 用 Codex provider registry 发现当前项目可用的 local peer skill providers。
5. 用 routing 层形成 skill_route 和 dispatch_decision。
6. 用 Codex 工具或 optional providers 收集本地或外部证据。
7. 按 result-recovery 回收到 sources、evidence、artifacts 或 boundaries。
8. 汇总角色观点、分歧、风险和行动建议。
9. 运行必要检查。
10. 明确哪些能力是 verified，哪些只是 planned。
